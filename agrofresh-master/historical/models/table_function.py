...
from django.db.models import (
    F, fields, ForeignObject, QuerySet, Manager, Model, NOT_PROVIDED
)
...
class TableFunctionQuery(Query):
    ...
    def _reorder_table_function_params(
        ...
        for key, arg in getattr(model, 'function_args').items():
            ...
            elif arg.initial != NOT_PROVIDED:
                ordered_function_params[key] = arg.initial
            else:
        ...
        if remaining:
            raise ValueError(f'Function arg `{remaining.pop()}` not found')
...
class TableFunctionManager(Manager):
    ...
    def table_function(self, **table_function_params: Dict[str, Any]) -> 'TableFunctionQuerySet':
        return self.get_queryset().table_function(**table_function_params)