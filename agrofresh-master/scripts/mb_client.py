...
def run_with_not_running_loop():
    ...
    new_loop, client = ModbusClient(schedulers.ASYNC_IO, host=HOST, port=PORT, loop=loop)
...
def run_with_already_running_loop():
    ...
    asyncio.set_event_loop(loop)
    loop, client = ModbusClient(schedulers.ASYNC_IO, host=HOST, port=PORT, loop=loop)
...
def run_with_no_loop():
    ...
    loop, client = ModbusClient(schedulers.ASYNC_IO, host=HOST, port=PORT)
    loop.run_until_complete(start_async_test(client.protocol))
    loop.close()
